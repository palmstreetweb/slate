 ...
 * Copy of neon/functions/storage-sign/authJwt.ts with a Vercel-flavoured URL lookup.
 ...
 * same HTTPS database URL the SPA is built with, which Vercel also exposes to
 * functions. Mirrors deriveNeonServiceUrls() in examples/_admin/neon/env.ts.
 */
export function jwksUrl(): string | null {
  ...
  try {
    ...
    const host = u.hostname.includes('.neonauth.')
      ? u.hostname
      : u.hostname.replace(/^([^.]+)\./, '$1.neonauth.');
    return `${u.protocol}//${host}${dbPath}/auth/.well-known/jwks.json`;
  } catch {
    return null;
  }
}
